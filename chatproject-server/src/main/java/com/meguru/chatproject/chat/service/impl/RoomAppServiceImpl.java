...
import com.meguru.chatproject.chat.domain.dto.RoomBaseInfo;
import com.meguru.chatproject.chat.domain.entity.*;
import com.meguru.chatproject.chat.domain.enums.GroupRoleAPPEnum;
...
import com.meguru.chatproject.chat.service.cache.*;
...
public class RoomAppServiceImpl implements RoomAppService {
    ...
    public ChatRoomResp getContactDetailByFriend(Long uid, Long friendUid) {
        ...
        AssertUtil.isNotEmpty(friendRoom, BusinessErrorEnum.FRIEND_NOT_EXIST);
    ...
    public MemberResp getGroupDetail(Long uid, long roomId) {
        ...
        Room room = roomCache.get(roomId);
        AssertUtil.isNotEmpty(roomGroup, BusinessErrorEnum.GROUP_ID_ERROR);
        Long onlineNum;
    ...
    public void delMember(Long uid, MemberDelReq request) {
        ...
        AssertUtil.isNotEmpty(member, BusinessErrorEnum.GROUP_ID_ERROR);
    ...
    public void addMember(Long uid, MemberAddReq request) {
        Room room = roomCache.get(request.getRoomId());
        AssertUtil.isNotEmpty(room, BusinessErrorEnum.GROUP_ID_ERROR);
        AssertUtil.isFalse(isHotGroup(room), BusinessErrorEnum.TOTAL_GROUP);
        RoomGroup roomGroup = roomGroupCache.get(request.getRoomId());
        AssertUtil.isNotEmpty(roomGroup, BusinessErrorEnum.GROUP_ID_ERROR);
        GroupMember self = groupMemberDao.getMember(roomGroup.getId(), uid);
        AssertUtil.isNotEmpty(self, BusinessErrorEnum.GROUP_NOT_EXIST);
